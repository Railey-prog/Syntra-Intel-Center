...
import { Bot, X, Send, User, Sparkles, Maximize2 } from "lucide-react";
...
export function FloatingChatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content:
        "Syntra neural network online. Ask me anything about AI-generated images, deepfakes, or media literacy.",
    },
  ]);
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className="fixed bottom-24 right-6 z-50 w-[340px] max-w-[calc(100vw-2rem)] rounded-2xl border border-border/60 bg-[#0d0d0d] shadow-2xl flex flex-col overflow-hidden"
          style={{ height: "490px" }}
        >
          {/* Header */}
          ...
          >
            ...
            {mutation.isPending && (
              <div className="flex gap-2.5 justify-start">
                ...
                <div className="bg-muted/40 border border-border/30 rounded-xl rounded-bl-sm px-4 py-3">
          ...
          {/* Suggested chips — always visible until user sends first message */}
          {messages.length === 1 && (
            <div className="px-3 pb-2 flex flex-wrap gap-1.5 flex-shrink-0">
              {SUGGESTED_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-[10px] px-2.5 py-1.5 rounded-full border border-border/50 bg-muted/20 text-muted-foreground hover:text-primary hover:border-primary/40 transition-colors flex items-center gap-1"
                >
          ...
          {/* Input */}
          <div className="px-3 pb-3 pt-2 border-t border-border/20 flex gap-2 items-end flex-shrink-0">
            <textarea
              ref={inputRef}
              value={input}
              ...
              rows={1}
              className="flex-1 resize-none bg-muted/20 border border-border/30 rounded-xl px-3 py-2.5 text-xs text-foreground placeholder:text-muted-foreground/40 focus:outline-none focus:border-primary/40 transition-all chatbot-scroll"
      ...
      {isOpen && (
        <button
          onClick={() => setIsOpen(false)}
          className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-[#1a1a1a] border border-border/40 flex items-center justify-center shadow-lg hover:bg-secondary/60 transition-colors"
